...
assert_equal(true, attrs[:is_favorite], "Author can see favorite status")
assert_equal(author.name, attrs[:author_name], "Author name is visible")
assert_equal(true, attrs[:can_edit], "Author can edit")
assert_equal(true, attrs[:can_view], "Author can view")
assert_not_nil(attrs[:word_count], "Author can see word count")
...
assert_nil(attrs[:is_favorite], "Partner cannot see favorite status")
assert_equal(author.name, attrs[:author_name], "Partner can see author name")
assert_equal(false, attrs[:can_edit], "Partner cannot edit")
assert_equal(false, attrs[:can_view], "Partner cannot view private content")
assert_nil(attrs[:word_count], "Partner cannot see metadata")
...
assert_nil(attrs[:is_favorite], "Partner cannot see author's favorite status")
assert_equal(author.name, attrs[:author_name], "Partner can see author name")
assert_equal(false, attrs[:can_edit], "Partner cannot edit author's note")
assert_equal(true, attrs[:can_view], "Partner can view shared content")
assert_not_nil(attrs[:published_at], "Partner can see publishing timestamp")
assert_not_nil(attrs[:first_shared_at], "Partner can see first shared timestamp")
...
assert_equal(false, attrs[:can_view], "Partner cannot view draft")
...
assert_equal(author.name, attrs[:author_name], "Other user sees author name for shared note")
assert_equal(false, attrs[:can_edit], "Other user cannot edit")
assert_equal(false, attrs[:can_view], "Other user marked as cannot view (not in couple)")
...
assert_equal("Anonymous", attrs[:author_name], "Anonymous sees 'Anonymous' as author")
assert_equal(false, attrs[:can_edit], "Anonymous cannot edit")
assert_equal(false, attrs[:can_view], "Anonymous cannot view")
...
assert_equal(true, attrs[:is_private], "Private note shows is_private flag")
assert_equal(false, attrs[:is_shared], "Private note shows is_shared as false")
assert_equal(false, attrs[:is_draft], "Private note shows is_draft as false")

serialized = NoteSerializer.new(shared_note).serializable_hash
attrs = serialized[:data][:attributes]

assert_equal(false, attrs[:is_private], "Shared note shows is_private as false")
assert_equal(true, attrs[:is_shared], "Shared note shows is_shared flag")
assert_equal(false, attrs[:is_draft], "Shared note shows is_draft as false")
...
assert_not_nil(attrs[:created_at], "Created at is always present")
assert_not_nil(attrs[:updated_at], "Updated at is always present")
assert_equal(author.id, attrs[:author_id], "Author ID is always present")
assert_equal(category.id, attrs[:category_id], "Category ID is always present")
assert_equal(check_in.id, attrs[:check_in_id], "Check-in ID is always present")
